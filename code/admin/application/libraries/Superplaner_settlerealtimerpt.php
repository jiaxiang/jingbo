...
class Superplaner_settlerealtimerpt_Core extends My
{
	//对象名称(表名)
    protected $object_name = 'superplaners_settle_realtime';
	...
	{
		$agents = ORM::factory('superplaners_settle_realtime')
	...
	{
		return ORM::factory('superplaners_settle_realtime')->count_all();
	}
	...
	public function agent_exist($data)
	{
		...
		$count = ORM::factory('superplaners_settle_realtime')->where($where)->count_all();
	...
	public function get_by_user_id($user_id)
	{
		...
		$user = ORM::factory('superplaners_settle_realtime')->where($where)->find();
		if($user->loaded){
	...
	{
		$list = array();
		$orm_instance = ORM::factory('superplaners_settle_realtime')
    	        ->select('users.email, users.lastname, superplaners_settle_realtimes.*')
    	        ->join('users', 'users.id', 'superplaners_settle_realtimes.user_id', 'LEFT');
    	// 处理输入条件
		$where = array();
		$in = array();
