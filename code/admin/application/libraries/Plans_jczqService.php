...
/*        if ($result['status'] != 0 || $result['status'] != 1 )
    ...
    public function bonus_plan($ordernum)
    {
        ...
        $add_money_total = $add_money_obj->get_bonus_add_money($total_bonus);
    ...
    * @return TRUE OR FALSE 有效或无效
    */
    public function bonus_plan_virtual($ordernum)
    {
    	$result = $this->get_by_order_id($ordernum);
    	...
    	$userobj = user::get_instance();
    	$usermoney = $userobj->get_user_virtual_money($result['user_id']);
    	$lan = Kohana::config('lan');
    	$moneyobj = user_money::get_instance();
    	...
    	if ($deduct_retmoney > 0)
    	{
    		...
    		$data_log['memo'] = $lan['money'][17].',方案ID:'.$basic_id_url;
    		$flagret = account_virtual_log::get_instance()->add($data_log);
    	...
    	$data_log['user_money'] = $usermoney;
    	$data_log['memo'] = $lan['money'][11].',方案ID:'.$basic_id_url;
    	$flagret = account_virtual_log::get_instance()->add($data_log);
    ...
    public function get_plans_result() {
    	...
    	$query = 'SELECT id,basic_id,play_method,codes FROM plans_jczqs where parent_id=0 and match_results is null';
    	//d($query, FALSE);
