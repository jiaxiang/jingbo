...
<head>
	...
	function del(id){
		if(window.confirm('您确信删除该记录吗?')){
			location.href = '/midwarecus/deleteMidwareCus.do?id=' + id;
    	}else{
    		return;
    	}
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td height="30" bgcolor="#F0F3F8" class="line2">
			&nbsp;&nbsp;&nbsp;&nbsp;当前位置:mid_midwarecus一览
		</td>
	</tr>
	<tr>
		<td  bgcolor="#FFFFFF"><br>
		<div align="left" style="width: 90%;margin:0 auto;">
<form id="iform" action="/midwarecus/listMidwareCus.do" method="post">
<table width="100%" align="center" cellpadding="1" border="0">
	<tr bgcolor="#ffffff">
		<td>接口调用key
			<input type="text" name="cusKeySear" id="cusKeySear" value="${cusKeySear}" />&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="submit" value="查询" />&nbsp;&nbsp;
			<input type="button" name="add" onclick="location.href = '/midwarecus/initAddMidwareCus.do';" value="添加"  />
		</td>
		<td>
		</td>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#5477C0">
	<tr bgcolor="#FFFFFF">
	    <td bgcolor="#F0F3F8">编号</td>
	    	    <td bgcolor="#F0F3F8">接口调用key</td>
	    	    <td bgcolor="#F0F3F8">接口调用密码</td>
	...
	<tr onmouseover="this.style.backgroundColor='#EDF6FF'" onmouseout="this.style.backgroundColor='#FFFFFF'" bgcolor="#FFFFFF">
	    <td>${status.count+(page.realSize-1)*pageSize}&nbsp;</td>
	    	    ...
	    	    <td>${e.cusPwd}&nbsp;</td>
	    	    <td>
	    	<a href='<%=request.getContextPath()%>/midwarecus/initUpdateMidwareCus.do?id=${e.id}' >更改</a>&nbsp;
...
<div class="MainPage PagePage">
		...
		<select id="pageSize" name="pageSize" onchange="javascript:document.getElementById('iform').submit();">
			...
			<option value="50" <c:if test="${param.pageSize == 50 || empty param.pageSize}">selected</c:if>>50</option>
		</select>条
</div>
