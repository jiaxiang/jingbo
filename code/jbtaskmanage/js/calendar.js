...
function setday(tt,obj) //主调函数
{
	...
	var ttReg = /^(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})$/;
	if(ttValue.length==19&&ttReg.test(ttValue)){
		ttValue = ttValue.substring(11,19);
		var ttValueArr = ttValue.split(":");
		meizzTheHour = ttValueArr[0];
		meizzTheMinute = ttValueArr[1];
		meizzTheSecond = ttValueArr[2];
	}
...
function meizzDayClick(n,ex)    //点击显示框选取日期，主输入函数*************
{
    ...
    if (hh<10&&hh.length==1)        {hh="0" + hh;}    //时
    if (mi<10&&mi.length==1)        {mi="0" + mi;}    //分
    if (se<10&&se.length==1)        {se="0" + se;}    //秒
...
{
	//add by gaojian   have problem to do(TODO)
	meizzTheDate = parent.meizzTheDate;
