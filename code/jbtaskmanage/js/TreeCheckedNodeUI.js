...
Ext.extend(Ext.ux.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
	...
	renderElements : function(n, a, targetNode, bulkRender){
        ...
        var buf = ['<li class="x-tree-node"><div ext:tree-node-id="',n.id,'" class="x-tree-node-el x-tree-node-leaf x-unselectable ', a.cls,'" unselectable="on">',
            ...
            cb ? ('<input class="x-tree-node-cb" type="checkbox" name="priid" value="'+n.id+'" ' + (a.checked ? 'checked="checked" />' : '/>')) : '',
    ...
    check : function(checked){
		...
		//this.checkbox.value = n.id;
		//alert(this.checkbox.value);
	...
	childCheck : function(node){
		...
		if(!a.leaf) {
			...
			for(var i = 0; i < cs.length; i++) {
				csui = cs[i].getUI();
				if(csui.checkbox.checked ^ a.checked) csui.check(a.checked);
			}
		}
	},
	// private
