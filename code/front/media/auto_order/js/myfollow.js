...
//var updatestate = function(gid,owner) {
//	parent.window.Y.postMsg('msg_login', function() {								
//    	window.top.location='/main/autobuy.html?type=new&lotid='+gid+'&owner='+owner;
//	});
//};
...
//	data += "&" + $_user.key.stime + "=" + stime;
	...
	$.ajax({
		...
		success : function(xml) {
			...
			if (code == "0") {
				...
				r.each(function() {
					...
					if ( state == 1 ) {
						...
						html += " <TD>"+adddate+"</TD>";
						html += ' <TD class=last_td><A href="#" onclick="setstate(\'' + id + '\',\'' + 0 + '\');">禁用</A> | <A  href="javascript:void 0" onclick="parent.Y.openUrl(\'/user_auto_order/add/'+lotyid+'/'+play_id+'/'+fid+'\',475,330)"  >修改</A></TD>';
					} else {
						html += " <TD class=last_td>禁用</TD>";
						html += " <TD>"+adddate+"</TD>";
						html += ' <TD class=last_td><A href="#" onclick="return setstate(\''+id+'\',\''+1+'\');">启用</A> </TD>';
					}
					html += " </TR>";
			...
			html += "<p>说明：<span class=\"gray\">自动跟单人可自愿取消自动跟单资格，也可以自由修改自动跟单设置，修改或者重新定制自动跟单，认购顺序按修改或重新定制后的时间顺序（排在后面）。</span> <a href=\"#\" target=\"_blank\" class=\"blue\">自动跟单规则&gt;&gt;</a> </p></div> ";
