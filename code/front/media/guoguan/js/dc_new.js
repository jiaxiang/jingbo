...
//	var lotid = location.search.getParam('lotid');
	...
	case "92":
	case "93":
		location.href ="/guoguan/jczq.html?lotid="+lotid;
		break;
	default:
		location.href ="/guoguan/index.html?lotid="+lotid;
		break;
	}
	...
	showinfo($("#lotid").val(), expect,"bjfs");
	...
	$("#expect").bind({
		change : function() {
			if ($("#seltype").val()=="my"){
				...
				loadmain($("#lotid").val(), $("#expect").val(),"bjfs");
			}
		}
	});	
			...
			if (code == "0") {
				...
				r.each(function() {
					...
					if (info.length>1){
						...
						html+='<td title="'+info[2].replace("1*1","单关").replaceAll("\\*","串")+'">'+info[2].split(",")[0].replace("1*1","单关").replaceAll("\\*","串")+'</td>';
...
var loadmain = function(lotid, expect,type) {
	...
	$.ajax({
		...
		success : function(xml) {
			...
			$("#fs_ps").val(rs.find("bjfs").attr("ps"));
			$("#fs_tp").val(rs.find("bjfs").attr("tp"));
			$("#fs_tr").val(rs.find("bjfs").attr("total"));
			
			$("#us_ps").val(rs.find("bjus").attr("ps"));
			$("#us_tp").val(rs.find("bjus").attr("tp"));
			$("#us_tr").val(rs.find("bjus").attr("total"));

			$("#ff_ps").val(rs.find("bjff").attr("ps"));
			$("#ff_tp").val(rs.find("bjff").attr("tp"));
			$("#ff_tr").val(rs.find("bjff").attr("total"));
			
			$("#uf_ps").val(rs.find("bjuf").attr("ps"));
			$("#uf_tp").val(rs.find("bjuf").attr("tp"));
			$("#uf_tr").val(rs.find("bjuf").attr("total"));
...
var loadpage = function(lotid, expect, type ,pn,ps,tp,tr) {
	...
	$.ajax({
		...
		success : function(xml) {
			...
			r.each(function() {
				...
				if(gnames.split(",").length>1){
					gstr=gnames.split(",")[0].replace("1*1","单关").replaceAll("\\*","串")+","+gnames.split(",")[1].replaceAll("\\*","串");
					if(gnames.split(",").length>2){gstr=gstr+"..";}
				}else{
					gstr=gnames.replace("1*1","单关").replaceAll("\\*","串");
				}
				...
				html+='<td class="bg_01">'+(parseFloat(bonus)>0?('<font color=red>'+parseFloat(bonus).rmb(false)+'</font>'):(parseFloat(bonus).rmb(false)))+'</td>';
				/*html+='<td>';
				html+=isdg?uid:'<a href="javascript: void 0" onclick="Y.openUrl(\'/zj/view/'+uid+'\',525,465)" >定制</a>';
				'</td>';*/
				html+='</tr>';				
		...
		error : function() {
			html+='<tr class="bg_01"><td  colSpan="9" style="color:red">过关文件尚未生成</td></tr></table>';
			$('#showlist').html(html);
			return false;			
		}
	});		
