...
//	var lotid = location.search.getParam('lotid');
	...
	case "88":
	case "89":
		location.href ="/guoguan/bjdc.html?lotid="+lotid;
		break;
	case "90":
		lotimg = "jczq_1.jpg";
		break;
	...
	$("#lotimg").attr('src', "/img/public/bt_icon/" + lotimg);
	$("#lotid").val(lotid);
			...
			if (code == "0") {
				...
				r.each(function() {
					...
					if (isdg){
						...
						html+='<a href="javascript:vh(\''+uid+'\',1)">'+uid+'</a>';
					}					
					html+='</td>'+
...
var loadmain = function(lotid, expect,type) {
	...
	$.ajax({
		url : "/data/guoguan/" + lotid + "/" + expect + "/" + expect + ".xml",
		type : "GET",
		dataType : "xml",
		cache : false,
		success : function(xml) {
...
var loadpage = function(lotid, expect, type ,pn,ps,tp,tr) {
	...
	$.ajax({
		url : "/data/guoguan/" + lotid + "/" + expect + "/jc" + type+"_"+pn+".xml",
		type : "GET",
		dataType : "xml",
		cache : false,
		success : function(xml) {
			//<row uid="用户名" ag="金星" au="银星" info="中奖注数，选择的场次，过关方式" bonus="奖金" betnum="投注注数" mnums="选择场数" zhushu="中奖注数" gnames="几串几" hid="方案id" />
			var r = $(xml).find("row");
			var j=0;
			...
			r.each(function() {
				...
				if (isdg){
					...
					html+='<a href="javascript: void 0" onclick="Y.openUrl(\'/main/viewjp.html?lotid='+lotid+'&uid='+uid+'&func=award\',477,465)" >'+uid+'</a>';
				}					
				html+='</td>'+'<td class="td_left">';
				...
				html+='<td class="bg_01">'+(parseFloat(bonus)>0?('<font color=red>'+parseFloat(bonus).rmb(false)+'</font>'):(parseFloat(bonus).rmb(false)))+'</td><td>';
				html+=isdg?uid:'<a href="javascript: void 0" onclick="$_sys.autobuy(\''+lotid+'\',\''+uid+'\')" >定制</a>';
				'</td></tr>';				
