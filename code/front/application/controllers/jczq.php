...
class Jczq_Controller extends Template_Controller {
	...
	/**
	 * 让球胜平负
	 * @param unknown_type $league
	 */
    public function rqspf($league = NULL) {
        $user = $this->_user;
        ...
        try 
        {
            ...
            $data['matchs'] = $objmatch->get_results($ticket_type, $play_type, null, $league);
            ...
            foreach ($data['matchs'] as $row)
            {
                ...
                $data['groups'][$row['match_date']][] = $row;
                $data['groups_date'][$row['match_date']] = $row['match_date'];
    		...
    		$view_url = Site_Service::get_instance()->echo_jczq_rqspf(); 
            $this->template = new View($view_url, $data);
    ...
    public function tobuy()
    {
        ...
        try {
            ...
            $this->template = new View('jczq/tobuy', $data);
            $this->template->set_global('_user', $this->_user);
    ...
    public function buyok()
    {
       ...
       try {
            ...
            $this->template = new View('jczq/buy_success', $data);
            $this->template->set_global('_user', $this->_user);
    ...
    public function viewdetail($orderid,$html_file="")
    {
        ...
        try 
        {
			...
			//d($data);
			if($html_file==""){
            	$this->template = new View('jczq/viewdetail', $data);	
