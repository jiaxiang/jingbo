<?php
class Settle_month_rpt_Controller extends Template_Controller 
{
...
//		}
        ...
        $total = $settlemonthrptDao -> count_itmes_with_condition($query_struct['where']);
...
//		$this->template->content = new View("distribution/settle_month_rpt_list");
...
//		$this->template->content->yesterday = date("Y-m-d",time()-24*3600);
		...
		$view = new View('distribution/settle_month_rpt_list', $data);
...
//		$view->set('yesterday', date("Y-m-d",time()-24*3600));
		...
		$view->set_global('_nav', 'settle_month_rpt');
		$view->render(TRUE);
	}
