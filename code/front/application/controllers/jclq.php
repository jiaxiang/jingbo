...
class Jclq_Controller extends Template_Controller {
   	 ...
   	 * @param unknown_type $search_date
   	 */
    public function sf($search_date = NULL)
    {
        $user = $this->_user;
        ...
        try 
        {
            ...
            foreach ($data['matchs'] as $row) 
            {
                ...
                empty($data['groups_date'][$weekday]) 
                    && $data['groups_date'][$weekday] = date("Y-m-d", strtotime(tool::get_date_byweek($weekday)));
     ...
     * 让分胜负
     */
    public function rfsf($search_date = NULL)
    {
        $user = $this->_user;
        $userobj = user::get_instance();
        $userobj->get_user_money($user['id']);
     ...
     * 胜负差
     */
    public function sfc($search_date = NULL)
    {
        $user = $this->_user;
        $userobj = user::get_instance();
        $userobj->get_user_money($user['id']);
     ...
     * 大小分
     */
    public function dxf($search_date = NULL)
    {
        $user = $this->_user;
        $userobj = user::get_instance();
        $userobj->get_user_money($user['id']);
...
//d($_POST,true);       
        ...
        try {
            ...
            foreach ($arrcode as $rowcode)
            {
                ...
                {
                    $data['result_type'] = Kohana::config('jclq');
                ...
                elseif ($data['play_method'] == 2)	//让分胜负
                {
                	$data['result_type'] = Kohana::config('jclq');

                    $spf_type = $data['result_type']['rfsf'];
                    $arr_cur_code = explode(",", $cur_code);
                ...
                {
                    $data['result_type'] = Kohana::config('jclq');
                    $bf_type = $data['result_type']['sfc']['result_type'];
                    $curcomb = json_decode($data['matchs'][$curmatch_id]['comb']);
                    $arr_cur_code = explode(",", $cur_code);
                    $arr_select = array();
                    ...
                    foreach ($arr_cur_code as $rowcur_code)
                    {
	                    $curnum = $curcomb->$rowcur_code;
	                    $arr_select[] = $bf_type[$rowcur_code]."(".$curnum->v.")";
                    }
                    $data['select'][$curmatch_id] = implode(",", $arr_select);
                }
                elseif ($data['play_method'] == 4)
                {
                    ...
                    foreach ($arr_cur_code as $rowcur_code)
                    {   
                        $code_value = $dxf_type['result_type'][$rowcur_code];
                        $curnum = $curcomb->{$code_value}->{'v'};
                        $arr_select[] = $dxf_type['result_cn'][$rowcur_code]."(".$curnum.")";
                    }
                    $data['select'][$curmatch_id] = implode(",", $arr_select);
...
//                    } 
            ...
            $this->template = new View('jclq/tobuy', $data);
            $this->template->set_global('_user', $this->_user);
    ...
    public function pute()
    {
        ...
        try {
            ...
            $plans_obj = Plans_jclqService::get_instance();
            
            //检查参数,验证客户端提交的价格,数量等是否正确
            //彩票拆分
            $testarr = array();
            $testarr = $plans_obj->
               get_tickets(999, $data['play_method'], $data['codes'], $data['typename'], $data['special_num'], $data['rate'], '999', TRUE);
    ...
    public function buyok()
    {
       ...
       try {
            ...
            $this->template = new View('jclq/buy_success', $data);
            $this->template->set_global('_user', $this->_user);
     ...
     public function my($status='ing')
    {
        ...
        try 
        {
            ...
            $data['status'] = $status;            
            $viewpage = 'jclq/my';
            $data['ajax_url'] ="/jclq/my_ajax/".$status."/20";
    ...
    public function my_ajax($status='ing', $page_size = 10)
    {
        ...
        try 
        {
            ...
            $base_url = url::base().'jclq/my_ajax/ing';
            if ($status=='ing')
            {
            ...
            {
                $query_struct_default['where']['status >'] = 2;
                $base_url = url::base().'jclq/my_ajax/end';
            }
            ...
            //调用服务执行查询
            $planobj = Plans_jclqService::get_instance();
            ...
            $ticket_type = $ticket_type['method'][6];
            ...
            foreach ($data['list'] as $row)
            {
                ...
                $list_html[$j]['column8'] = "<a href=\"/jclq/viewdetail/".$row['basic_id']."\" target=\"_blank\">详情</a>";
                $j++;
                $i++;
            }
	...
	public function viewdetail($orderid,$html_file="")
	{
        ...
        try 
        {
            ...
            $matchobj = match::get_instance();
            $aPlan = $plans_jclq_obj->get_by_order_id($orderid);
            ...
            if ($aPlan['parent_id'] > 0)
            {
                $aPlan = $plans_jclq_obj->get_parent_id($aPlan['parent_id']);
            }
            
            $arrcode = explode('/', $aPlan['codes']);            //扯分选项
            
            //胆码
            if (!empty($aPlan['special_num']))
            {
                $arrspenum = explode('/', $aPlan['special_num']);
                foreach ($arrspenum as $rowspenum)
                {
                    $curspenum = explode('|', $rowspenum);
                    $curspenumselect = explode('[', $curspenum[1]);
            ...
            $emptyodds = FALSE;
            $data['odds'] = $plans_jclq_obj->get_odds_by_ordernum($aPlan);    //获取所选的赔率,当无最新赔率时下面从赛事表获取            
           
            if ($aPlan['play_method'] == 2 || $aPlan['play_method'] == 4){
	            $data['goallines'] = $plans_jclq_obj->get_goallines_by_ordernum($aPlan);
// d($data['goallines'], false);
            }
			if (empty($data['odds'])){
                $emptyodds = TRUE;
            }
//d($emptyodds,false);
            $jclq_config = Kohana::config('jclq');
          
            $data['match_cnchar'] = array();        //赛事选择参数对应的显示文字 key value格式
            $data['match_cnchar_r'] = array();      //赛事赛果参数对应的显示文字 key value格式
            
            if ($aPlan['play_method'] == 1)
            {
                $data['match_cnchar'] = $jclq_config['spf']['result_cn'];
                $data['match_cnchar_r'] = ($data['match_cnchar']);
            }
            elseif ($aPlan['play_method'] == 2)
            {
                $data['match_cnchar'] = array();
                $data['match_cnchar_r'] = array_flip($jclq_config['rfsf']['result_cn2']);
//                $data['match_cnchar'] = $jclq_config['rfsf']['result_cn'];
//                $data['match_cnchar_r'] = array_flip($data['match_cnchar']);
            }
            elseif ($aPlan['play_method'] == 3)
            {
				$data['match_cnchar'] = array();
//				$data['match_cnchar_r'] = array();
//				$data['match_cnchar'] = array();
				$data['match_cnchar_r'] = $jclq_config['sfc']['result_type'];
            }
            elseif ($aPlan['play_method'] == 4)
            {
                $data['match_cnchar'] = array();
//                $data['match_cnchar'] = array_flip($jclq_config['dxf']['result_cn']);
                $data['match_cnchar_r'] = array_flip($jclq_config['dxf']['result_cn2']);
            }
            ...
            foreach ($arrcode as $rowcode)
            {
                ...
                $data['match_groups'][$curmatch_id]['match'] = $matchobj->get_match($curmatch_id, 6, $aPlan['play_method']);   //存储赛事信息
  
                if ($emptyodds)
                {
                    if ($aPlan['play_method'] == 1) 
                    {
                    	$data['odds'][$curmatch_id.'|'.$data['match_groups'][$curmatch_id]['match']['match_num'].'[2]'] = $data['match_groups'][$curmatch_id]['match']['H'];
                        $data['odds'][$curmatch_id.'|'.$data['match_groups'][$curmatch_id]['match']['match_num'].'[1]'] = $data['match_groups'][$curmatch_id]['match']['A'];
                    }
                    elseif ($aPlan['play_method'] == 2) 
                    {
						$comb = json_decode($data['match_groups'][$curmatch_id]['match']['comb']);
                        $data['odds'][$curmatch_id.'|'.$data['match_groups'][$curmatch_id]['match']['match_num'].'[2]'] = $comb->h->v;
                        $data['odds'][$curmatch_id.'|'.$data['match_groups'][$curmatch_id]['match']['match_num'].'[1]'] = $comb->a->v;
                    }
                    elseif ($aPlan['play_method'] == 3) 
                    {
                        $result_type = $jclq_config['sfc']['result_type'];
                        $comb = json_decode($data['match_groups'][$curmatch_id]['match']['comb']);
                        foreach ($result_type as $rowkey => $rowtype)
                        {
	                        $data['odds'][$curmatch_id.'|'.$data['match_groups'][$curmatch_id]['match']['match_num'].'['.$rowkey.']'] = $comb->$rowkey->v;
                        }
                    }
                    elseif ($aPlan['play_method'] == 4) 
                    {
						$comb = json_decode($data['match_groups'][$curmatch_id]['match']['comb']);
                    	$data['odds'][$curmatch_id.'|'.$data['match_groups'][$curmatch_id]['match']['match_num'].'[1]'] = $comb->{'h'}->{'v'};
                        $data['odds'][$curmatch_id.'|'.$data['match_groups'][$curmatch_id]['match']['match_num'].'[2]'] = $comb->{'a'}->{'v'};
                    }
                }
            ...
            $data['detail'] = $aPlan;
            $data['user'] = $userobj->get($aPlan['user_id']);
            
            //是否可以合买
            $data['is_open'] = FALSE;
            if ($aPlan['plan_type'] == 1)
            {
                if ($aPlan['status'] == 0 && $aPlan['zhushu'] > $aPlan['buyed'] && strtotime($aPlan['time_end']) > time())
                {
                    if (!empty($aPlan['friends']))
                    {
                        if (!empty($this->_user))
                        {
                            $arruser = explode(',', $aPlan['friends']);
            ...
            if ($this->_user['id'] == $aPlan['user_id']) {
            	$data['join_data'] = true;
            }
            else {
            	$data['join_data'] = $plans_jclq_obj->find_join_data($this->_user['id'],$aPlan['id']); //是否有参与过本方案
            }
            $data['is_public'] = $plans_jclq_obj->plan_open($data['detail'], $data['join_data']);
			...
			$data['jclqcfg'] = $jclq_config;
			if($html_file==""){
            	$this->template = new View('jclq/viewdetail', $data);	
			}elseif($html_file=="join"){
            	$this->template = new View('jclq/viewdetail_join', $data);			
			}            
    ...
    public function ajax_data_lists_hm($page_size = "")
    {
        ...
        try {
			$Plans_jclq_obj = Plans_jclqService::get_instance();
			$data_list = $Plans_jclq_obj->query_assoc($carrier_query_struct);//数据列表
			$data_count = $Plans_jclq_obj->query_count($carrier_query_struct);//总记录数		
			...
			if ($data_list)
			{
				foreach($data_list as $key=>$value)
				{
				   ...
				   if ($value['surplus'] == 0)
				   {
					...
					$col_username = "<span style='float:left;display:block;width:70px;height:25px;overflow:hidden;' title='".$user['lastname']."'>".$user['lastname']."</span><a style='float:right;*float:right;margin-right:5px;display:block;color:#db8a19' href='javascript:return 0;' onclick='dingzhi(".$value['user_id'].",6,".$value['play_method'].");'>定制跟单</a>";
					$list_html[]=array(
						"column0"=>$value['id'],				
						"column1"=>($key+1),
						...
						"column5"=>'<a href="/jclq/viewdetail/'.$value['basic_id'].'" target="_blank">查看</a>',
						"column6"=>$baodi_text,
						...
						"column9"=>'<input type="button" value="参与" class="btn_Dora_s m-r" id="b_'.$value['basic_id'].'"/><a href="/jclq/viewdetail/'.$value['basic_id'].'" target="_blank">详情</a>',
						);				
				}

				$base_url = "jclq/ajax_data_lists_hm/";
				$config['base_url'] = $base_url.$total_rows;		
				$config['total_items'] = $data_count;       //总数量
				$config['query_string'] = 'page';
				$config['items_per_page'] = $total_rows;	//每页显示多少第		
	...
	{
	    //更新赛果
	    Plans_jclqService::get_instance()->get_plans_result();
	}    
    
	/**
     * 自动兑奖 copy from 'controllers/jclq.php'
     */
	public function auto_get_price() 
	{
//		echo 'auto_get_price in';
	    //更新方案结果
	    Plans_jclqService::get_instance()->get_plans_result();
	    
	    //兑奖
		Plans_jclqService::get_instance()->bonus_count();
//		echo 'auto_get_price out';
	}
	...
	public function bonus_info($order_num)
	{
        ...
        $this->template = new View('jclq/bonus_info');
	...
	public function cancel_plan($order_num) {
		...
		header("Location: /jclq/viewdetail/".$order_num);
		exit();
	}
