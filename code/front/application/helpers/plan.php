...
class plan_Core {
    ...
    private $detail_uri_dlt;
    private $detail_uri_plw;
    private $detail_uri_qxc;
    private $detail_uri_pls;
    ...
    {
        $this->detail_url_jczq = '/jczq/viewdetail/';
        $this->detail_url_jclq = '/jclq/viewdetail/';
        $this->detail_url_bjdc = '/bjdc/viewdetail/';
        $this->detail_url_zcsf = '/zcsf/viewdetail/';
        $this->detail_uri_dlt = '/dlt/view/';
        $this->detail_uri_plw = '/plw/view/';
        $this->detail_uri_qxc = '/qxc/view/';
        $this->detail_uri_pls = '/pls/view/';
    }
        ...
        switch ($result['ticket_type']) 
        {
            case 1:
               ...
               if (!empty($result['detail']))
               {
                   ...
                   $result['plan_my_money'] = $result['detail']['my_copies'] * $result['detail']['price_one'];
            ...
            case 2:
               ...
               if (!empty($result['detail']))
               {
                   ...
                   $result['plan_my_money'] = $result['detail']['money'];
                   if ($result['detail']['parent_id'] > 0)
                   {
            ...
            case 7:
               ...
               if (!empty($result['detail']))
               {
                   ...
                   $result['detail']['expect'] = $result['detail']['issue'];
                   $result['plan_my_money'] = $result['detail']['my_copies'] * $result['detail']['price_one'];
                   if ($result['detail']['parent_id'] > 0)
                   {
            ...
            break;
            case 8:
            	$result['detail'] = $this->get_result_lotty($result['id']);
            	if (!empty($result['detail']))
            	{
            		$result['detail']['total_price'] = $result['detail']['allmoney'];//总价格
            		$result['plan_copies'] = $result['detail']['allmoney']/$result['detail']['pmoney'];         //总份数
            		...
            		$result['plan_detail'] = $this->detail_uri_dlt.$result['detail']['id'];
            		$result['plan_my_money'] = $result['detail']['sale']['rgmoney'];
            		$result['detail']['expect'] = $result['detail']['qihao'];
            		$result['detail']['price'] = $result['detail']['sale']['rgmoney'];
            		$result['detail']['progress'] = $result['detail']['renqi'];
            	}
            	break;
            case 9:
            	$result['detail'] = $this->get_result_lotty($result['id']);
            	if (!empty($result['detail']))
            	{
            		...
            		$result['plan_buyed'] = $result['detail']['rgnum'];           //已购买数量
            		$result['plan_bonus'] = $result['detail']['afterbonus'];           //奖金
            		$result['plan_detail'] = $this->detail_uri_plw.$result['detail']['id'];
            		$result['plan_my_money'] = $result['detail']['sale']['rgmoney'];
            		$result['detail']['expect'] = $result['detail']['qihao'];
            		$result['detail']['price'] = $result['detail']['sale']['rgmoney'];
            		$result['detail']['progress'] = $result['detail']['renqi'];
            ...
            case 10:
            	...
            	if (!empty($result['detail']))
            	{
            		...
            		$result['plan_buyed'] = $result['detail']['rgnum'];           //已购买数量
            		$result['plan_bonus'] = $result['detail']['afterbonus'];           //奖金
            		$result['plan_detail'] = $this->detail_uri_qxc.$result['detail']['id'];
            		$result['plan_my_money'] = $result['detail']['sale']['rgmoney'];
            		$result['detail']['expect'] = $result['detail']['qihao'];
            		$result['detail']['price'] = $result['detail']['sale']['rgmoney'];
            		$result['detail']['progress'] = $result['detail']['renqi'];
            ...
            case 11:
            	...
            	if (!empty($result['detail']))
            	{
            		...
            		$result['plan_buyed'] = $result['detail']['rgnum'];           //已购买数量
            		$result['plan_bonus'] = $result['detail']['afterbonus'];           //奖金
            		$result['plan_detail'] = $this->detail_uri_pls.$result['detail']['id'];
            		$result['plan_my_money'] = $result['detail']['sale']['rgmoney'];
            		$result['detail']['expect'] = $result['detail']['qihao'];
            		$result['detail']['price'] = $result['detail']['sale']['rgmoney'];
            		$result['detail']['progress'] = $result['detail']['renqi'];
     ...
     *
     */
    public function get_result_lotty($id,$is_orderid = TRUE){
    	if ($is_orderid)
    	{
