...
class ticket_operation_Core {
    ...
    {
    	//return self::$instance->change_code_jczq(&$arrcode, &$arrtype, $maxcode = 0, $special_num);
...
//d($arrtype, true);
        ...
        if (in_array($user['id'], $test_users)) {*/
	        if (count($arrtype) > 1) {
	        	$rules_ex = Kohana::config('rule_mxn.jczq_ex');
	        	$match_count = count($arrcode);
	        	$type_count = count($arrtype);
	        	$type_comb_name = '';
	        	for ($i = 0; $i < $type_count; $i++) {
	        		$t = explode('串', $arrtype[$i]);
	        		$type_comb_name .= $t[0];
	        	}
	        	if ($type_comb_name != '' && 
	        	isset($rules_ex[$match_count][$type_count][$type_comb_name])) {
//d($rules_ex[$match_count][$type_count][$type_comb_name], false);
	        		$arrtype = explode(',', $rules_ex[$match_count][$type_count][$type_comb_name]);
	        	}
	        }
        //}
//d($match_count.'/m:'.$type_count.'/c:'.$type_comb_name, true);
        if($tmp_danguan === '单关'){
        	array_splice($arrtype, 0,0, '单关');	
        }
    }
