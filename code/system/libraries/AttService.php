...
class AttService_Core {
    ...
    public function __construct($img_dir_name = '')
    {
        ...
        $this->dir_name = ATTPATH.$this->img_dir_name.'/';
        $this->no_img_file = ATTPATH.'no_img.gif';
    }
    
    /** 
     * 保存上传的默认原始图片
     */ 
    public function save_default_img($file_tmp_name, $default_img_id=NULL) 
    {
        require_once(Kohana::find_file('vendor', 'phpthumb/ThumbLib.inc',TRUE));
        $default_img = PhpThumbFactory::create($file_tmp_name);
        $default_img_id || $default_img_id = md5($file_tmp_name.uniqid(rand(), true));
        stripos($default_img_id,'_') && $default_img_id = strtr($default_img_id, array('_'=>''));
        $default_img_path = $this->dir_name.$default_img_id.'/';
        if(is_dir($default_img_path))
        {
            //清理旧的文件
            $this->delete_dir_file($default_img_path, false);
        }
        else
        {
            @mkdir($default_img_path, 0777, true);
        }
    ...
    public function get_img_dir($img_id=NULL, $w=0, $h=0, $type='jpg') 
    {
        ...
        {
            //默认图片
            $img = $this->dir_name.$img_id.'/'.$this->default_img_name.'.'.$this->default_img_type;
     ...
     * 生成缩略图 
     */ 
    private function thumb($img_id, $w=120, $h=90, $type='jpg') 
    {
         //require_once(Kohana::find_file('vendor', 'phpthumb/ThumbLib.inc',TRUE));
         $img_path = $this->dir_name.$img_id;
         $default_img = $img_path.'/'.$this->default_img_name.'.'.$this->default_img_type;
         $thumb_img = $img_path.'/'.$w.'_'.$h;         
         $this->resize_image($default_img, $w, $h, $thumb_img, $type);
         return true;
