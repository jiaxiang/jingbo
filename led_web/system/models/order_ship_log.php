...
class Order_ship_log_Model extends ORM {
    public function validate(array & $array, $save = FALSE, & $errors) {
        ...
        $array = Validation::factory($array)
            ->pre_filter('trim')
			->add_rules('ship_num','required','length[1,200]')
            ...
            ->add_rules('ems_num','required','length[0,255]')
            ->add_rules('send_data','required','length[0,1024]')
            ->add_rules('content_admin','length[0,1000]')
            ->add_rules('content_user','length[0,1000]')
            ->add_rules('ship_status_id','required','numeric')
