...
class Ticketnum_Controller extends Template_Controller {
	...
	public function index($status = 'all') {
		...
		/* 搜索功能 */
		$search_arr      = array('id','codes','order_num','port');
        ...
        $total_money = 0;
        if ($status == 'ticket_tj') {
	        $query = 'select sum(money) as total_money from ticket_nums where status > 0';
		...
		$this->template->content = new View("order/order_ticketnum", $return_data);
