...
class News_Controller extends Template_Controller {
	...
	{	
		...
		{ 
            //标签过滤
            tool::filter_strip_tags($_POST, array('content','content_en'));
	...
	public function logo_upload()
	{
		...
		if($_POST)
		{
            ...
            $img_id = $AttService->save_default_img($_FILES['img_val']["tmp_name"]);
            ...
            if(!$img_id){
				...
				echo "}";
				echo "  window.parent.tinyMCE.execCommand('insertimage',false,'http://".$site_config['name'].$AttService->get_img_url($img_id,0,0)."');\n";
				//echo "  window.parent.tinyMCE.execCommand('insertimage',false,'".$AttService->get_img_url($img_id,0,0)."');\n";
