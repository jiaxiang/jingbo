...
(function($){
	...
	$.fn.Validform.sn={
		...
		checkform:function(obj,parentobj,tiptype,show){//show用来判断是表达提交还是blur事件引发的检测;
			...
			if(obj.attr("recheckno")){
				var theother=parentobj.find("input[name="+obj.attr("recheckno")+"]:first");
				if(obj.val()==theother.val()){
					errorobj=obj;
					var errormsg_recheckno=obj.attr("errormsg_recheckno") || tipmsg.w;
					this.showmsg(errormsg_recheckno,tiptype,{obj:obj},show);
					return false;
				}
			}
