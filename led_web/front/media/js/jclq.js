		...
		_upSp: function (arrSp){
            ...
            switch(ptype){
				...
				case 'sfc' : 
					...
					break;
				default    : val2xml = {'2':'lost','1':'win'}; //sf,rfsf
            }
        ...
        addMsg: function (){
            ...
            this.onMsg('msg_ggtype_change', function (type, isduochuan){
                ...
                if (type.zy.length>0 && parseInt(type.zy[0]) == all.size()) {
                    ...
                    if ((type.zy.length>0 && parseInt(type.zy[0]) -1 == dan.length) || (this.C('issfc') && dan.length>1) || dan.length ==  Math.min(7, all.size()-1)) {
    ...
    Class('Buy', {
        index:function (){
            ...
            this.get('#gobuy,#gohm').click(function (e, Y){//提交
				...
				if (!Y.checkMaxTickets(data, ggData)) {
        ...
        _upView: function (data, hand){//更新视图
           ...
           // this.get('#buy_money').html('...');
            ...
            this.delayGetPrixTimer = setTimeout((function() {
				...
				this.get('#imaxmoney').val(prix === undefined ? 'underfined' : parseFloat(prix*bs)); //理论最大money
			...
			//这句话是用来显示投注明细的
		    this.postMsg('msg_get_bottom_info');  //用于显示投注明细的js；
