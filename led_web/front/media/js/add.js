...
function() {
    ...
    clickpopjs;
    creElm({
        //href: 'http://union..com/ushare/code/css/css1.css',
        rel: 'stylesheet',
        type: 'text/css'
    },
    'link');
    $CKE = {
        ...
        over: function() {
                ...
                timerCont = setInterval(function() {
                    if (div.innerHTML) {
                        ...
                        with(div.style) {
                            ...
                            left = p.l + 'px';
                            if (p.l + div.offsetWidth > wh.w) {
                                p.l = p.l - (div.offsetWidth - T.offsetWidth + 10);
                                left = p.l + 'px';
                            }
                            T.style.display = a
                        };
            ...
            if (!clickpopjs) {
                clickpopjs = creElm({
                    src: 'http://union..com/ushare/code/ckepop.js',
                    charset: 'gbk'
                },
                'script', head);
        ...
        center: function() {
			...
			if (!this.script) {
                this.script = creElm({
                    //src: 'http://union..com/ushare/code/ckecenterpop.js',
                    charset: 'gbk'
                },
                'script', head);
    ...
    for (var i = 0, ci, tmp; ci = h[i++];) {
        if (/\bushare\b/.test(ci.className)) {
        ...
        if (ci.className && (tmp = ci.className.match(/^ushare_button_(\w+)$/)) && tmp[1]) {
            ci.innerHTML = '<span class="jtico button jtico_' + tmp[1] + '" style="cursor:pointer;">' + ci.innerHTML + '</span>';
...
function ushare_sendto(a) {
    ...
    //window.open('http://union..com/ushare/send/?webid=' + a + '&url=' + e(conf.url || location) + '&title=' + e((conf.title?conf.title:document.title)) + '&pic=' + e($('div .detail img:nth-child(1)')&&$('div .detail img:nth-child(1)').attr('src')?$('div .detail img:nth-child(1)').attr('src'):'')+ (conf.un ? '&un=' + conf.un: ''), '');
    return false
}
function jt_copyUrl() {
