...
class Superplaner_settlerealtimerptdtl_Core extends My
{
	//对象名称(表名)
    protected $object_name = 'superplaners_settle_realtime_dtl';
	...
	{
		return ORM::factory('superplaners_settle_realtime_dtl')->count_all();
	}
	...
	public function agent_exist($data)
	{
		...
		$count = ORM::factory('superplaners_settle_realtime_dtl')->where($where)->count_all();
	...
	public function get_by_user_id($user_id)
	{
		...
		$user = ORM::factory('superplaners_settle_realtime_dtl')->where($where)->find();
		if($user->loaded){
	...
	{
		$list = array();
		$orm_instance = ORM::factory('superplaners_settle_realtime_dtl')
    	        ->select('uuu.lastname as clientlastname, agent.lastname as agentlastname, superplaners_settle_realtime_dtls.*')
    	        ->join('users as agent', 'agent.id', 'superplaners_settle_realtime_dtls.agentid', 'LEFT')
    	        ->join('users as uuu', 'uuu.id', 'superplaners_settle_realtime_dtls.user_id', 'LEFT');
    	// 处理输入条件
		$where = array();
		$in = array();
	...
	public function sum_items($query_struct=array(), $m_flag = 90)
	{
		$list = array();
		$orm_instance = ORM::factory('superplaners_settle_realtime_dtl')
    	        ->select('sum(superplaners_settle_realtime_dtls.fromamt) asum, sum(superplaners_settle_realtime_dtls.client_retamt) bsum')
    	        ->join('users as agent', 'agent.id', 'superplaners_settle_realtime_dtls.agentid', 'LEFT')
    	        ->join('users as uuu', 'uuu.id', 'superplaners_settle_realtime_dtls.user_id', 'LEFT')
    	        ->where('superplaners_settle_realtime_dtls.flag<>'.$m_flag);
    	// 处理输入条件
		$where = array();
		$in = array();
		if(isset($query_struct['where'])&&is_array($query_struct['where']))
