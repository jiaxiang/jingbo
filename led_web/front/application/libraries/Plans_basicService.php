...
class Plans_basicService_Core extends DefaultService_Core 
{
    ...
    public function get_count_notend($uid)
    {
        $obj = ORM::factory('plans_basic')
                    ->where("user_id", $uid)
                    ->where("date_add >", date("Y-m-d H:i:s", mktime(0,0,0,date("m")-3, date("d"), date("Y"))))
                    ->where("status <", 3);
        return $obj->count_all();
    }
    ...
    public function get_hemai_data($userid, $ticket_type, $lasttime) {
    	...
    	$query = 'select id,order_num,ticket_type from plans_basics where plan_type=1 and start_user_id="'.$userid.'"'.$ticket_type.' and status=2 and agmretstatus=3 and agrretstatus=3 and date_add>="'.$lasttime.'"';
    ...
    public function update_settle_status($id) {
    	...
    	$db_obj = Database::instance();
    	$query = 'update plans_basics set agmretstatus=2 ,agrretstatus=2 '.$where.' and agmretstatus=3 and agrretstatus=3';
    	//d($query, false);
    	$results = $db_obj->query($query);
    }
    
    public function update_settle_status_order_num($id) {
    	if (is_array($id) == true) {
    		$where = 'where order_num in ('.implode(',', $id).')';
    	}
    	else {
    		$where = 'where order_num="'.$id.'"';
    	}
    	$db_obj = Database::instance();
    	$query = 'update plans_basics set agmretstatus=2 ,agrretstatus=2 '.$where.' and agmretstatus=3 and agrretstatus=3';
    	//d($query, false);
    ...
    public function is_cancel_plan($order_num, $userid) {
    	$plan_basic_info = $this->get_by_ordernum($order_num);
    	//d($plan_basic_info);
    	if ($plan_basic_info['status'] != 0 ) {
    		return false;
    	}
    	if (time() > strtotime($plan_basic_info['date_end'])) {
    		return false;
    	}
    	if ($plan_basic_info['plan_type'] != 1) {
    		return false;
    	}
    	//$userid = $this->_user['id'];
    	if ($userid != $plan_basic_info['start_user_id']) {
    		return false;
    	}
    	return true;
    }
