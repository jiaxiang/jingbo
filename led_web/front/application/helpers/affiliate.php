...
class affiliate_Core {
	...
	public static function send($order_id)
	{
		...
		$order['order_product_detail'] = Myorder_product_detail::instance()->order_product_details(array('order_id'=>$order['id']));
...
说明：
订单号   订单总价（扣除税收和邮费）    产品列表    数量列表     产品对应价格列表    短货币编码 来源联盟id

产品列表，数量列表，产品对应价格列表以竖线（|）分割，开始结束不包括该分割线
例如：
