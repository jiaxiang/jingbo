...
class user_money_Core 
{
     ...
     * 根据输入的订单号返回资金结构体百分比
     */
    public function get_stru_by_order_num($order_num)
    {
        //获取资金关联id
        $obj = ORM::factory('account_log');
        $obj->where('order_num', $order_num)->find();
        ...
        foreach ($result as $row)
        {
            $return[$row->log_type] = $row->price / $allprice;
        }
        
        if (empty($return['USER_MONEY']))
        {
            $return['USER_MONEY'] = 0;
        }        
        if (empty($return['BONUS_MONEY']))
        {
            $return['BONUS_MONEY'] = 0;
        }
        if (empty($return['FREE_MONEY']))
        {
            $return['FREE_MONEY'] = 0;
        }        
        return $return;
