...
class News_Controller extends Template_Controller {
	...
	//新闻详细页
	public function news_detail($id,$play=''){
		$view = new  View('/news_en/news_detail');
		$view->set_global('_topnav','is_news');
		$view->set_global('_user', $this->_user);
        $view->set_global('_nav', NULL);
		...
		$view->set("news_classify",$news_classify['category_name_en']);
		...
		//关键字 相关资讯
		$key=explode("|",$news_infor[0]['key_en']);
		$view->set("key",$key);
		//print_r($key);
		$arr=array();
		if($key){
	...
	public function news_list($id){
		$view = new View('/news_en/news_list');
		$view->set_global('_user', $this->_user);
        $view->set_global('_nav', NULL);
		...
		//d($classRow);
		if(count($classRow)){
		 $str=$classRow[0]['category_name_en'];
			 if($classRow[0]['parent_id']>0){
				...
				$str= '<a href="/en/news/news_list/'.$classRow[0]['parent_id'].'">'.$cate[0]['category_name_en'].'</a> > '.$str;
			 }
		}
		...
		$config['base_url'] = "/en/news/news_list/".$id;
