...
class Cases_Controller extends Template_Controller {
	...
	//新闻详细页
	public function case_detail($id,$play='') {
		$view = new  View('/cases_en/case_detail');
		...
		$view->set("news_classify_name",$news_classify['category_name_en']);
		//获取上一条记录
	...
	//新闻列表页
	public function cases_list($id) {
		$view = new  View('/cases_en/cases_list');
		$view->set_global('_user', $this->_user);
        $view->set_global('_nav', NULL);
		...
		if(count($classRow)){
		 $str=$classRow[0]['category_name_en'];
			 if($classRow[0]['parent_id']>0){
				...
				$str= '<a href="#">'.$cate[0]['category_name_en'].'</a> > '.$str;
			 }
		}
		...
		$config['base_url'] = "/en/cases/cases_list/".$id;
